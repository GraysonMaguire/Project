...
def work(P0, V0, M0, t, dt, colRad, minParticles):
    ...
    for i in (range(iterations)):
        if i > 0:
            rawP[i], rawM[i], V = checkForCollision(
                rawP[i], rawM[i - 1], V, colRad)
        # if i > 0:
        #     rawM[i] = rawM[i - 1]

        F = calcForceOnParticles(rawP[i], rawM[i])
        ...
        V = calcNextVelocity(V, F, rawM[i], dt)

        rawP[i + 1], pHalf = calcNextPosition(rawP[i], V, dt)

        checkForEscape(pHalf, V, rawM[i])
